...
exports.run = async (client, message, args) => {
    ...
    let msg = await message.reply("Ne tür verileri sıfırlamak istersiniz? `(tümünü,ses ve mesaj) bunlardan birisini yazmalısın.");
    ...
    if(!reply){
        message.reply("15 Saniye boyunca cevap vermediğin için komut iptal edildi !")
        return delete_Messages(deleteMessages);
    }
    deleteMessages.push(reply);

    if(!["tümünü", "ses", "mesaj"].some(type => reply.content.toLowerCase() == type)) return delete_Messages(deleteMessages);
    ...
    message.reply(`\`${reply.content}\`Veriler başarıyla silindi.`).then(m => m.delete({timeout: 5000}));
};

exports.conf = {
    commands: ["statsil","veriıfırla","statsıfırla","ss"],
    enabled: true,
    guildOnly: true
};
